...
object StevesData {
  val percs = Map(1 -> List(0.5, 0.7, 0.8, 0.85, 0.9),
    ...
    12 -> List(0.95, 1, 1.1, 1.16, 1.2)
  )

  val reps = Map(1 -> List(5, 5, 5, 5, 3),
    2 -> List(8, 8, 5, 5, 3),
    3 -> List(8, 8, 5, 5, 3),
    4 -> List(8, 5, 5, 3, 3),
    5 -> List(8, 5, 5, 3, 3),
  ...
  def weights(name: String, week: Int)(f: Scores => Double): List[(Int, Double, Int)] = {
    val ns: Scores = nums.getOrElse(name, Scores(0,0,0,0,0))
    val ps: List[Double] = percs.getOrElse(week, List())
    val rs: List[Int] = reps.getOrElse(week, List())

    val prs = rs zip ps

    prs.map{ case (r,p) => (r, p, (p * f(ns)).toInt) }
  }
